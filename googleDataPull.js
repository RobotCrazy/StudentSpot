...
function makeApiCall(subject) {
  ...
  requestTeacher.then(function (response) {
    ...
    createTeacherCourseArray(response.result, subject);
  }, function (reason) {
...
function studentFeedbackApiCall(courseList) {
  var paramsStudent = {
    // The ID of the spreadsheet to retrieve data from.
    ...
    for (var i = 0; i < courseList.length; i++) {
      ...
      for (var x = 0; x < response.result.values.length; x++) {
        if (courseName == response.result.values[x][1]) {
          ...
          //console.log(courses[i].studentFeedback.challenge);
        }
      }
    }
  }, function (reason) {
...
function createTeacherCourseArray(result, subject) {
  ...
  studentFeedbackApiCall(courses);
