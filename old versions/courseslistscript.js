...
function buildTeacherDesc(teacher) {
	...
	var recomPrereq = document.createTextNode(teacher.prerequisites.recommended);
	recomPrereqPar.appendChild(recomPrereq);

	var reqPrereqPar = document.createElement("p"); //required prerequisites
	var reqPrereq = document.createTextNode(teacher.prerequisites.required);
...
var course1 = new Course({teacherThing:"5",otherThing:"6"},"a","a","a","a","a","a","a","a");
...
function displayCourses(subject) {
	...
	for (var i = 0; i < courses.length; i++) {
		...
		for (var x = 0; x < courses[i].credits.length; x++) {
		...
		var teacherDesc = buildTeacherDesc(courses[i].teacher);
		var studentDesc = buildStudentDesc(courses[i].student);
...
function toggleCourseDisplay() {
	var course = this.parentNode.parentNode;
	...
	if (coursePanel.className.includes("state-hidden")) {
		...
		style.type = "text/css";
		var courseOpenAnimation = "\ @keyframes openCourse { \
			from { max-height:0px; }" + "to {max-height:" + coursePanel.scrollHeight + "px; } } ";
		style.innerHTML += courseOpenAnimation;
		coursePanel.appendChild(style);*/
